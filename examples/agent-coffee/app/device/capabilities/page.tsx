...
function DeviceCapabilitiesContent() {
        ...
        <div className="w-full max-w-sm rounded-lg border border-border bg-card shadow-md p-6 space-y-5">
          <div className="text-center space-y-1">
            ...
            <p className="text-[13px] text-foreground/40">
              {isSignUp ? "Create an account to approve this agent." : "Sign in to approve this agent."}
            </p>
          </div>
          {code && (
            <div className="p-3 rounded-md border border-border bg-foreground/[0.02] text-center">
              <span className="text-[9px] font-semibold text-foreground/35 tracking-wider uppercase">Code</span>
          ...
          <p className="text-center text-[12px] text-foreground/40">
            ...
            <button onClick={() => setIsSignUp(!isSignUp)} className="underline cursor-pointer">{isSignUp ? "Sign in" : "Sign up"}</button>
          </p>
        </div>
      </div>
    );
  ...
  if (sessionPending || loading) return <div className="min-h-dvh flex items-center justify-center"><Spinner /></div>;
  if (!agentId) return <div className="min-h-dvh flex items-center justify-center"><p className="text-[13px] text-foreground/40">Missing agent_id parameter</p></div>;
  if (error && !agentInfo) return <div className="min-h-dvh flex items-center justify-center"><p className="text-[13px] text-red-500">{error}</p></div>;
    ...
    return (
      <div className="min-h-dvh flex items-center justify-center">
        <div className="text-center space-y-3">
          <div className={`mx-auto h-12 w-12 rounded-full flex items-center justify-center ${approved ? "bg-emerald-50 dark:bg-emerald-950/30" : "bg-red-50 dark:bg-red-950/30"}`}>
            {approved
              ? <svg className="h-5 w-5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
          ...
          <h1 className="text-[18px] font-semibold">{approved ? "Access Approved" : "Access Denied"}</h1>
    ...
    <div className="min-h-dvh flex items-center justify-center px-4">
      <div className="w-full max-w-md rounded-lg border border-border bg-card shadow-md overflow-hidden">
        <div className="text-center px-5 pt-5 pb-3">
          ...
          <p className="text-[13px] text-foreground/40 mt-1">Review the capabilities this agent is requesting.</p>
        </div>
        {code && (
        ...
        <div className="mx-5 my-3 rounded-md border border-border overflow-hidden">
          <div className="px-3.5 py-2.5 bg-foreground/[0.02]">
            ...
            <p className="text-[11px] text-foreground/35 font-mono mt-0.5">{agentInfo?.agent.mode}</p>
          </div>
          {pendingGrants.length > 0 && (
            <div className="px-3.5 py-3 space-y-1.5 border-t border-border">
              ...
              {pendingGrants.map((g) => (
                <div key={g.id} className="p-2 rounded-md border border-border bg-foreground/[0.02]">
        ...
        {error && <div className="mx-5 mb-3 px-3 py-2 rounded-md border border-red-200 bg-red-50 dark:bg-red-950/30 text-red-500 text-[13px]">{error}</div>}
        <div className="px-5 py-3.5 border-t border-border bg-foreground/[0.02] flex gap-1.5">
          <button onClick={() => handleAction("deny")} disabled={actionState !== "idle"}
          ...
          <button onClick={() => handleAction("approve")} disabled={actionState !== "idle"}
            ...
            {actionState === "approving" ? <span className="flex items-center justify-center gap-1.5"><Spinner /> Approving...</span> : "Approve"}
          </button>
        </div>
      </div>
    </div>
...
export default function DeviceCapabilitiesPage() {
  return (
    <Suspense fallback={<div className="min-h-dvh flex items-center justify-center"><Spinner /></div>}>

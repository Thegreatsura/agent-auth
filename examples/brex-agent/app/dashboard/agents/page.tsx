...
function StatusDot({ status }: { status: string }) {
  const color =
    ...
    status === "revoked" ? "bg-red-400" : "bg-white/20";
...
function formatConstraintValue(field: string, value: unknown): string {
  ...
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    if (field === "amountCents" && typeof value === "number") return `${label}: $${(value / 100).toFixed(2)}`;
...
function MetaItem({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-0.5 rounded-lg bg-white/[0.04] px-3 py-2">
      <span className="text-[10px] uppercase tracking-wider text-white/25 font-medium">{label}</span>
    ...
    <div className="rounded-lg bg-white/[0.04] px-3 py-2 hover:bg-white/[0.06] transition-colors">
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <code className="text-[12px] font-mono text-white/70 truncate block">{grant.capability}</code>
          {grant.reason && (
            <p className="text-[11px] text-white/30 italic truncate mt-0.5">&ldquo;{grant.reason}&rdquo;</p>
          )}
        </div>
        <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-medium ${
          grant.status === "active" ? "bg-emerald-500/15 text-emerald-400"
          : grant.status === "pending" ? "bg-amber-500/15 text-amber-400"
      ...
      {hasConstraints && (
        <div className="mt-1.5 ml-[34px] flex flex-wrap gap-1">
          {Object.entries(grant.constraints!).map(([field, value]) => (
            <span key={field} className="inline-flex items-center gap-0.5 rounded bg-white/[0.06] px-1.5 py-0.5 text-[10px] text-white/40 font-mono">
      ...
      {loading ? (
        <div className="space-y-2">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-16 rounded-lg border border-white/[0.08] bg-white/[0.02] animate-pulse" />
          ))}
        </div>
      ) : agents.length === 0 ? (
        <div className="rounded-lg border border-white/[0.08] border-dashed py-16 text-center">
          ...
          <p className="mt-1 text-[12px] text-white/25">Agents will appear here once they connect.</p>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`rounded-lg border transition-all ${
                  isExpanded ? "border-white/[0.15] bg-white/[0.02]" : "border-white/[0.08] hover:border-white/[0.12]"
                }`}
              >
                <button
                ...
                >
                  <div className={`flex items-center justify-center h-8 w-8 rounded-full shrink-0 ${
                    agent.status === "active" ? "bg-emerald-500/15" : agent.status === "pending" ? "bg-amber-500/15" : "bg-white/[0.06]"
                  }`}>
                    <AgentIcon className={`h-4 w-4 ${
                      agent.status === "active" ? "text-emerald-400" : agent.status === "pending" ? "text-amber-400" : "text-white/25"
                    }`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-medium ${
                        ...
                        : agent.status === "pending" ? "bg-amber-500/15 text-amber-400"
                        : agent.status === "revoked" ? "bg-red-500/15 text-red-400"
                        : "bg-white/[0.06] text-white/30"
                      }`}>
                        <StatusDot status={agent.status} />
                        {agent.status}
                      </span>
                      <span className="text-[10px] text-white/20 bg-white/[0.04] rounded px-1.5 py-0.5">{agent.mode}</span>
                    </div>
                  ...
                  <div className="border-t border-white/[0.06] px-4 py-4">
                    ...
                    <div className="mb-4">
                      ...
                      {agent.agent_capability_grants.length > 0 ? (
                        ...
                        <p className="text-[12px] text-white/25 py-2">No capabilities granted yet.</p>
                      )}
                    </div>
